-- =============================================================================
-- Factura autoimpresor: registro fiscal por venta.
--
-- Guarda el numero fiscal correlativo (EST-PEXP-0000001) asignado a cada venta
-- cuando el modo de facturacion es 'autoimpresor', junto a un snapshot de los
-- datos que exige la factura legal (timbrado, vigencia, condicion, liquidacion
-- de IVA). El correlativo se toma/incrementa de empresa_autoimpresor_config
-- (numero_actual) de forma atomica al emitir.
--
-- Single-schema (ferreteriarepublica). Aditiva e idempotente. No toca SIFEN ni
-- el ticket interno existente.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS ferreteriarepublica.factura_autoimpresor (
  ...
  -- Snapshot de la liquidacion de IVA al momento de emitir (montos IVA incluido).
...
-- Correlativo unico por timbrado + establecimiento + punto de expedicion.
